...
pub async fn upload(
    ...
    // Check for duplicate in DB (idempotent — same content = same UUID)
    let existing: Vec<Value> = state
        .db
        .query("SELECT * FROM file_resource WHERE cas_uuid = $id LIMIT 1")
        .bind(("id", cas_id.clone()))
        .await
        .map_err(db_err)?
        .take(0)
        .map_err(db_err)?;

    let file_record = if let Some(existing) = existing.into_iter().next() {
        info!(
