...
(function() {
    ...
    function init() {
      ...
      // Make RMA frame invisible
      makeRmaFrameInvisible();
    ...
    function removeLoginMessage() {
      const loginMessageDivs = document.querySelectorAll('div[style*="background-color: #fff7e6"]');
      loginMessageDivs.forEach(div => {
        div.remove();
      });
    }
    
    // Make RMA frame invisible
    function makeRmaFrameInvisible() {
      const rmaInput = document.getElementById('rma');
      if (rmaInput) {
        rmaInput.style.border = 'none';
        rmaInput.style.backgroundColor = 'transparent';
        rmaInput.style.fontWeight = 'bold';
        rmaInput.style.fontSize = '22px';
      }
    }
    ...
    function addAddressLogicExplanation() {
      ...
      if (devicesContainer) {
        ...
        addressInfo.innerHTML = `
          <p><strong>Device Return Information:</strong></p>
          <ul style="margin-top: 5px; padding-left: 20px;">
            <li>By default, all devices are shipped to the address specified in the "Billing Information" section.</li>
            <li>You can specify an alternative return address for any device.</li>
            <li>If you haven't specified an address for a device, the nearest address above in the list will be used.</li>
          </ul>
        `;
